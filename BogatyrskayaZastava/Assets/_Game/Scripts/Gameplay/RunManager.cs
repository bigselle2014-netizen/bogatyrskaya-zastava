...
namespace BogatyrskayaZastava.Gameplay
{
    ...
    public class RunManager : MonoBehaviour
    {
        ...
        private void OnWaveCompleted(WaveCompletedEvent evt)
        {
            ...
            // BUG-016: инкрементируем сами, не берём из события — защита от рассинхрона
            _currentWave++;
        ...
        private void TransitionToDeckChoice()
        {
            ...
            if (ServiceLocator.TryGet<DeckBuilder>(out var deckBuilder))
            {
                ...
                // BUG-008: пустой пул (не назначены карточки в Inspector) — ран зависнет в DeckChoice
                if (choices.Count == 0)
                {
                    Debug.LogWarning("[RunManager] BUG-008: DeckBuilder вернул 0 вариантов. Пропускаем выбор колоды.");
                    OnDeckChoiceComplete();
                    return;
                }
