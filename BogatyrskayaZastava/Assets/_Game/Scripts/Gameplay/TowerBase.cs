...
namespace BogatyrskayaZastava.Gameplay
{
    ...
    public class TowerBase : MonoBehaviour
    {
        ...
        private readonly List<EnemyBase> _targetsBuffer = new List<EnemyBase>(16); // instance field — static вызывал shared state между башнями
        ...
        private void TryReviveNearestDeadTower(float hpPercent)
        {
            ...
            for (int i = 0; i < towers.Count; i++)
            {
                ...
                float dist = Vector3.Distance(transform.position, t.transform.position);
                if (dist > myRange) continue; // UD-02: radius check (ZN-3 Белояр range=99 → вся карта)
                if (dist < nearestDist)
                {
                    nearestDist = dist;
                    nearestDead = t;
