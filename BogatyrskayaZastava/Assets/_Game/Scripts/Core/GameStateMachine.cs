        ...
        private static void OnEnterState(GameState state, GameState previousState)
        {
            switch (state)
            {
                ...
                case GameState.Loading:
                    // QA-007 КРИТИЧНО: SceneLoader не реализован — игра зависнет здесь
                    // Unity Dev Middle: реализовать SceneLoader.LoadScene(targetScene) — неделя 3-4
                    Debug.LogError("[StateMachine] Loading state: SceneLoader не реализован! Игра зависнет.");
                    break;

                case GameState.Tutorial:
                    // QA-008: Tutorial не инициализируется — Unity Dev Middle, неделя 5
                    Debug.LogWarning("[StateMachine] Tutorial state: Tutorial.cs не инициализирован.");
                    break;

                case GameState.Gameplay:
                    // QA-009 КРИТИЧНО: WaveController и HUD не инициализируются
                    // Unity Dev Middle: инициализировать EnemyWaveController, GameHUD — неделя 4
                    Debug.LogError("[StateMachine] Gameplay state: WaveController и HUD не инициализированы!");
