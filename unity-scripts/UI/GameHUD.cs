...
namespace BogatyrskayaZastava.UI
{
    public class GameHUD : MonoBehaviour
    {
        ...
        private void OnWaveCompleted(WaveCompletedEvent evt)
        {
            ...
            float waitTime = 12f;
            if (ServiceLocator.TryGet<EnemyWaveController>(out var waveCtrl))
                waitTime = waveCtrl.TimeBetweenWaves;
        ...
        /// <summary>
        /// Нажатие кнопки "Начать волну" — форсирует старт следующей волны
        /// </summary>
        public void StartWaveButtonPressed()
        {
            ...
            if (ServiceLocator.TryGet<EnemyWaveController>(out var waveController))
            {
        ...
        private IEnumerator WaveTimerCountdown(float seconds)
        {
            ...
            if (ServiceLocator.TryGet<EnemyWaveController>(out var waveCtrl2))
            {
                waveCtrl2.StartNextWave();
            }
        }
    }
}
